...
- **Was anything actually shipped or attempted?** A ticket can be tagged "Release Note" without any real customer-facing change ever happening — e.g. the *original requested change itself* was investigated and concluded "not a bug" / "working as intended" / "won't fix" / "duplicate, see ticket X", so nothing was built or altered. If the thread shows this, the correct verdict is **REJECT-NOT-SHIPPED**: this ticket should not get a release note at all this month.
  - **Do not confuse this with a secondary investigation that clears a different, unrelated concern.** It is common for a comment thread to contain both (a) confirmation that the actual requested change shipped and works, and (b) a follow-up investigation into some other discrepancy (often against a different system) that gets explained away as "not a bug" *in that other thing* — not a statement that the original change didn't happen. Read for what changed on Hotelchamp's own side specifically; a comment saying "X is correct on our end, the mismatch is actually caused by [other system] doing something different" is evidence the fix *did* ship, not evidence to reject. Only use REJECT-NOT-SHIPPED when the ticket's own requested change is the thing being declared not-a-bug/no-op.
